...
const FilterBar = ({ 
  ...
  <div className="bg-white rounded-xl shadow-sm border border-gray-200 p-4 mb-6">
    ...
    {/* ✅ PROMINENT: Coming Soon / Available Toggle Buttons */}
    <div className="mb-4 pb-4 border-b border-gray-100">
      <label className="block text-xs text-gray-500 mb-2 font-medium">Availability Status</label>
      <div className="flex flex-wrap gap-2">
        <button
          type="button"
          onClick={() => handleFilterChange('comingSoon', '')}
          className={`flex items-center gap-2 px-4 py-2 rounded-lg text-sm font-medium transition-all border-2 ${
            filters.comingSoon === ''
              ? 'bg-[#708268] text-white border-[#708268] shadow-md'
              : 'bg-white text-gray-700 border-gray-200 hover:border-[#708268] hover:bg-[#708268]/5'
          }`}
        >
        ...
        <button
          type="button"
          onClick={() => handleFilterChange('comingSoon', 'coming-soon')}
          className={`flex items-center gap-2 px-4 py-2 rounded-lg text-sm font-medium transition-all border-2 ${
            filters.comingSoon === 'coming-soon'
              ? 'bg-amber-500 text-white border-amber-500 shadow-md'
              : 'bg-white text-gray-700 border-gray-200 hover:border-amber-500 hover:bg-amber-50'
          }`}
        >
        ...
        <button
          type="button"
          onClick={() => handleFilterChange('comingSoon', 'available')}
          className={`flex items-center gap-2 px-4 py-2 rounded-lg text-sm font-medium transition-all border-2 ${
            filters.comingSoon === 'available'
        ...
        <div className="flex gap-4">
          ...
          <div className="flex-1 min-w-0">
            ...
            <div className="flex flex-wrap items-center gap-2 mb-1.5">
              ...
              {/* ✅ Coming Soon / Available Badge */}
              {product.comingSoon ? (
                <span className="flex-shrink-0 text-[10px] px-2 py-0.5 bg-amber-100 text-amber-700 rounded-full flex items-center gap-0.5 font-medium border border-amber-300">
                  <Clock className="w-2.5 h-2.5" />
                  Coming Soon
                </span>
              ) : (
                <span className="flex-shrink-0 text-[10px] px-2 py-0.5 bg-emerald-100 text-emerald-700 rounded-full flex items-center gap-0.5 font-medium border border-emerald-300">
            ...
            <div className="flex flex-wrap items-center justify-between gap-2">
              ...
              <div className="flex items-center gap-1.5">
                ...
                {canDuplicate && (
                  <button
                    onClick={() => onDuplicate(product._id, product.productName)}
...
export default function AdminAllProducts() {
  ...
  const fetchProducts = async () => {
    ...
    try {
      ...
      // ✅ Coming Soon filter
      if (filters.comingSoon === 'coming-soon') {
        queryParams.append('comingSoon', 'true');
      } else if (filters.comingSoon === 'available') {
        queryParams.append('comingSoon', 'false');
      }
  ...
  const handleDuplicateClick = (id, name) => {
    setDuplicateModal({ show: true, id, name });
  };

  const handleDuplicateConfirm = async () => {
    if (!duplicateModal.id) return;
    
    setIsDuplicating(true);
    try {
      const token = localStorage.getItem('token');
      const response = await fetch(`http://localhost:5000/api/products/${duplicateModal.id}/duplicate`, {
        method: 'POST',
        headers: {
          'Authorization': `Bearer ${token}`,
          'Content-Type': 'application/json'
      ...
      if (data.success) {
        toast.success(
          `Product duplicated successfully! New SKU: ${data.info?.newSkuCode || 'generated'}`,
          { duration: 5000 }
        );
        fetchProducts();
      } else {
          ...
          <div className="bg-white rounded-xl shadow-xl max-w-md w-full mx-4 border border-gray-200">
            <div className="p-6">
              ...
              <p className="text-sm text-gray-600 mb-2">
                ...
                <span className="font-semibold text-gray-900">"{duplicateModal.name}"</span>?
              </p>
              
              <div className="bg-blue-50 border border-blue-200 rounded-lg p-3 mb-6">
                <p className="text-xs font-medium text-blue-800 mb-2">📋 What will be copied:</p>
                <ul className="text-xs text-blue-700 space-y-0.5 mb-3">
                  <li>✓ All product details (name, description, price)</li>
                  <li>✓ Images, variants & add-ons</li>
                  <li>✓ Category, brand & tags</li>
                  <li>✓ Stock, costs & metadata</li>
                </ul>
                <p className="text-xs font-medium text-amber-700 mb-2">🔕 What will be cleared:</p>
                <ul className="text-xs text-amber-700 space-y-0.5 mb-3">
                  <li>• Barcode (empty - assign manually)</li>
                  <li>• FAQs (empty)</li>
                </ul>
                <p className="text-xs font-medium text-gray-700 mb-2">🆕 What will be new:</p>
                <ul className="text-xs text-gray-600 space-y-0.5">
                  <li>• New SKU code (auto-generated)</li>
                  <li>• New product name with "(Copy)" suffix</li>
                  <li>• Starts as <strong>Inactive</strong> (enable when ready)</li>
                </ul>
              </div>

              <div className="flex items-center justify-end gap-3">
                <button
                  onClick={() => setDuplicateModal({ show: false, id: null, name: '' })}
                ...
                <button
                  ...
                  disabled={isDuplicating}
                  className="flex items-center gap-2 px-4 py-2 text-sm font-medium text-white bg-blue-600 hover:bg-blue-700 rounded-lg transition-colors shadow-md disabled:opacity-50"
                >
                  {isDuplicating ? (
                    <>
