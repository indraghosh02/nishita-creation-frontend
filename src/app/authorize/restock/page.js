...
//   onEdit,
// }) {
//   const [qtyInput, setQtyInput] = useState(String(item.addQuantity || 1));

//   useEffect(() => {
//     setQtyInput(String(item.addQuantity || 1));
//   }, [item.addQuantity]);
...
//   const newStock = item.currentStock + (item.addQuantity || 1);
//   const addedCount = item.addQuantity || 1;

//   return (
//     <div className="border border-black/15 rounded-xl p-3 bg-white hover:border-black/30 transition-all">
...
//             )}
//             {addedCount > 1 && (
//               <span className="text-[9px] px-1.5 py-0.5 rounded-full bg-green-100 text-green-700 border border-green-200 font-medium flex items-center gap-1">
...
//             <span className="font-bold text-black">{newStock}</span>
//           </div>
//         </div>
...
//               onClose();
//             }}
//             className="mt-2 p-2 bg-white rounded-lg border border-gray-200 hover:border-black/40 cursor-pointer transition-all flex items-center justify-between"
//           >
...
//     // Try to match a variant / sub-variant by barcode / sku
//     let matched = null;
...
//     // Has variants, barcode matched parent → ask user
//     toast.info(
...
//         // ✅ Already in list → increment by `quantity` (default 1)
//         if (existing) {
//           const newQty = (existing.addQuantity || 1) + quantity;
//           toast.success(`+${quantity} → ${displayLabel} (now ${newQty})`, {
//             duration: 1800,
//           });
//           return prev.map((it) =>
//             it === existing
...
//                   ...it,
//                   addQuantity: newQty,
//                   currentStock, // refresh stock from server
//                 }
//               : it
//           );
//         }

//         // ✅ New item → starts at quantity (default 1)
//         const newItem = {
...
//     setItems((prev) =>
//       prev.map((it) =>
//         it._id === id ? { ...it, addQuantity: (it.addQuantity || 1) + 1 } : it
//       )
//     );
...
//       prev.map((it) =>
//         it._id === id
//           ? { ...it, addQuantity: Math.max(1, (it.addQuantity || 1) - 1) }
//           : it
//       )
//     );
...
//     if (items.length === 0) {
//       toast.error('Add at least one product to restock');
//       return;
//     }
...
//       if (json.success) {
//         toast.success(
//           `Stock updated for ${json.data?.updatedCount || items.length} item(s)`,
...
//   const totalUnits = items.reduce((sum, it) => sum + (it.addQuantity || 1), 0);
...
//                     Each scan adds +1 to the list. Adjust quantities as needed.
//                   </p>
//                 </div>
//               </div>
...
//                 <strong>How it works:</strong> Scan a barcode (or click a search
//                 result) to <strong>+1</strong> the quantity. Scan the same
//                 product again → it increments by 1 more. Use <strong>+ / −</strong>{' '}
//                 or type a number to add extra units.
//               </div>
//             </div>
...
//                       {[scan.productName, scan.variantName, scan.subVariantName]
//                         .filter(Boolean)
//                         .join(' / ')}
//                     </span>
...
//                   ({totalItems} item{totalItems !== 1 ? 's' : ''}, {totalUnits}{' '}
//                   unit{totalUnits !== 1 ? 's' : ''})
//                 </span>
//               </div>
//               {items.length > 0 && (
...
//                   disabled={saving}
//                   className="flex items-center gap-2 px-6 py-2.5 bg-black text-white rounded-xl hover:bg-[#485442] transition-all text-sm font-medium shadow-md disabled:opacity-50"
//                 >
//                   {saving ? (
...
//                   )}
//                   {saving ? 'Updating Stock...' : 'Confirm Restock'}
//                 </button>
//               </div>
//             </div>
//           </div>
...
//   // For simple products: DB already has the +1 saved → currentStock is the new stock
//   // For variants: preview = currentStock + pending addQuantity
//   const newStockPreview = isVariant
//     ? item.currentStock + addedCount
//     : item.currentStock;
...
//             )}
//             {addedCount > 0 && (
//               <span className="text-[9px] px-1.5 py-0.5 rounded-full bg-blue-100 text-blue-700 border border-blue-200 font-medium flex items-center gap-1">
//                 <FaArrowUp className="w-2 h-2" />
//                 +{addedCount} pending
//               </span>
//             )}
...
//       {/* Quantity controls — visible for ALL rows */}
//       <div className="flex items-center justify-between gap-3 mt-3 pt-3 border-t border-gray-100 flex-wrap">
//         <div className="flex items-center gap-2 flex-wrap">
//           <label className="text-xs font-medium text-gray-600">
//             {isVariant ? 'Add Quantity:' : 'Extra Quantity:'}
//           </label>
...
//       {!isVariant && (
//         <div className="mt-2 pt-2 border-t border-green-100 text-[11px] text-green-700 flex items-center gap-1.5">
//           <FaCheckCircle className="w-3 h-3" />
//           +1 already saved (Stock: <strong>{item.currentStock}</strong>).
//           Add extra via <strong>+</strong> button.
//         </div>
//       )}
//     </div>
//   );
...
//     // VARIANT matched → add to list only
//     if (matched) {
//       toast.dismiss(toastId);
//       handleVariantAdd({
//         product,
...
//     // Variant product, barcode matched parent
//     toast.info(
//       `"${product.productName}" has variants. Pick one from search.`,
//       { id: toastId }
//     );
...
//   // ============================================================
//   // ✅ Simple product: auto-save +1, add to list with addQuantity = 0
//   // ============================================================
...
//       if (idx >= 0) {
//         // Already in list — update stock, keep addQuantity as-is
//         const updated = [...prev];
//         updated[idx] = {
//           ...updated[idx],
//           currentStock:
...
//       // New row — addQuantity starts at 0 (extra quantity only)
//       return [
//         ...prev,
//         {
...
//           addQuantity: 0, // ✅ starts at 0 for simple products
//         },
//       ];
//     });
...
//   // ============================================================
//   // Variant product: add to list only (bulk-save later)
//   // ============================================================
...
//           selectedColor,
//           currentStock,
//           addQuantity: 1, // ✅ variants start at 1
//         },
//       ];
//     });
...
//     if (isVariant) {
//       // Just bump addQuantity (bulk-save on submit)
//       setItems((prev) =>
//         prev.map((it) =>
//           it._id === id
...
//     // Simple product — auto-save extra +1
//     const saveResult = await saveToBackend({
...
//           ? {
//               ...it,
//               addQuantity: (it.addQuantity || 0) + 1,
//               currentStock:
//                 typeof newStock === 'number'
//                   ? newStock
//                   : it.currentStock + 1,
...
//     toast.success(`+1 → ${item.productName}`, { duration: 1200 });
//   };
...
//     const isVariant = !!(item.variantId || item.subVariantId);
//     const currentQty = item.addQuantity || 0;
...
//       );
//     } else {
//       // Simple product — just reduce display counter
//       setItems((prev) =>
//         prev.map((it) =>
//           it._id === id
//             ? {
//                 ...it,
//                 addQuantity: Math.max(0, (it.addQuantity || 0) - 1),
//               }
//             : it
//         )
//       );
//       toast.info(
//         'Backend stock is already updated. Edit the product to reduce stock.',
//         { duration: 3000 }
//       );
//     }
//   };
...
//       if (
//         !confirm(
//           "This product's stock was already saved to the database. Removing it from the list won't undo the stock change. Continue?"
//         )
//       ) {
//         return;
//       }
...
//   // ============================================================
//   // Submit — only variant rows (simple products already saved)
//   // ============================================================
//   const handleSubmit = async () => {
...
//       toast.info('All items already saved. Nothing to submit.');
//       return;
//     }
...
//           { id: toastId, duration: 4000 }
//         );
//         // Remove only variant rows with addQuantity > 0
//         setItems((prev) =>
//           prev.filter(
//             (it) =>
//               !(
...
//                   <strong>Simple products</strong> → +1 saved instantly on scan.
//                   Use +/− to add more.
//                 </p>
//                 <p>
//                   <strong>Variant products</strong> → pick a variant; submit
//                   when ready.
//                 </p>
//               </div>
//             </div>
  ...
  return (
    ...
    >
      ...
      <div className="flex items-center justify-between gap-3 mt-3 pt-3 border-t border-gray-100 flex-wrap">
        ...
        <div className="flex items-center gap-2">
          {/* Save Extra button — only for simple products with pending extra */}
...
export default function RestockPage() {
  ...
  // ============================================================
  // Simple product: auto-save +1, list row starts at 0
  // ============================================================
  const handleSimpleProductAdd = async (product) => {
    const productId = product._id;
  ...
  // ============================================================
  // Variant product: add to list only
  // ============================================================
  const handleVariantAdd = ({ product, variant, subVariant }) => {
    const productId = product._id;
    const variantId = variant?.id || variant?._id || null;
  ...
  // ============================================================
  // Quantity change handlers — NO auto-save for simple products
  // ============================================================
  const increaseQty = (id) => {
    setItems((prev) =>
      prev.map((it) =>
  ...
  // ============================================================
  // Save Extra Quantity (only for simple products)
  // ============================================================
